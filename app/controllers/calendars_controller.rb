class CalendarsController < ApplicationController
    ...
    TIME_ZONE = 'Japan'
      ...
      def puts_event(event)
        ...
        puts "ID:       #{event.id}"
        puts "Start:    #{event.start&.present? ? event.start.date_time : nil}"
        puts "End:      #{event.end&.present? ? event.end.date_time : nil}"
        puts "StartDate:    #{event.start&.date.present? ? event.start.date : nil}"
        puts "EndDate:      #{event.end&.date.present? ? event.end.date : nil}"
        puts "attendees:      #{event.attendees.present? ? event.attendees : nil}"
        puts "description #{event.description.present? ? event.description : nil}"
        # puts "OriginalStartTime #{event.originalStartTime.datetime.present? ? event.originalStartTime.datetime : nil}"
        # puts "OriginalStartTime #{event.originalStartTime.datetime.present? ? event.originalStartTime.datetime : nil}"
      end
      ...
      def set_event(summary, description, location, start_time, end_time)
        Google::Apis::CalendarV3::Event.new({
            summary: summary.present? ? summary : nil,
            description: description.present? ? description : nil,
            location: location.present? ? location : nil,
            start: Google::Apis::CalendarV3::EventDateTime.new(
              date_time: start_time.present? ? start_time : nil
            ),
            end: Google::Apis::CalendarV3::EventDateTime.new(
              date_time: end_time.present? ? end_time : nil
            )
          }
        )
      end
      ...
      def read
        ...
        events = @service.list_events(@calendar_id,
                                      time_min: (Time.new(2021, 12, 1)).iso8601,
                                      time_max: (Time.new(2021, 12, 31)).iso8601,
                                     )
